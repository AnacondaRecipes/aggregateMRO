...
if [[ $target_platform =~ linux.* ]] || [[ $target_platform == win-32 ]]; then
  ...
  mkdir -p $PREFIX/lib/R/library/stringdist
  mv * $PREFIX/lib/R/library/stringdist
      ...
      for libdir in lib/R/lib lib/R/modules lib/R/library lib/R/bin/exec sysroot/usr/lib; do
        pushd $libdir || exit 1
          for SHARED_LIB in $(find . -type f -iname "*.dylib" -or -iname "*.so" -or -iname "R"); do
            ...
            install_name_tool -change /Library/Frameworks/R.framework/Versions/3.5/Resources/lib/libomp.dylib "$PREFIX"/lib/libomp.dylib $SHARED_LIB || true
          done
        popd
      done
    popd
